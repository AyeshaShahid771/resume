...
function TimelineCard({ exp, index }: { exp: typeof experiences[0], index: number }) {
  ...
  return (
    <div className={`relative flex w-full mb-24 justify-center items-center ${isRight ? "md:justify-start md:pl-[50%]" : "md:justify-end md:pr-[50%]"}`}>
      {/* Timeline Dot */}
      <div className="absolute left-1/2 -translate-x-1/2 w-4 h-4 rounded-full z-20 bg-[#121212] border-2" style={{ borderColor: exp.accent }}>
        <motion.div
      ...
      >
        {exp.badge && (
          <span 
            className="inline-block px-3 py-1 rounded-full text-[9px] font-syne tracking-widest mb-4 bg-blue-500/10 text-blue-400 border border-blue-500/20"
          >
            {exp.badge}
          </span>
        )}
        
        <h3 className="font-playfair italic text-3xl text-white mb-2">{exp.company}</h3>
        <p className="font-syne uppercase text-[11px] tracking-wider mb-6" style={{ color: exp.accent }}>
          {exp.role}
        </p>
        ...
        <ul className="space-y-3">
          {exp.points.map((point, i) => (
            <li key={i} className="flex gap-3 text-sm text-white/50 leading-relaxed group">
              <span className="text-white/20 transition-colors group-hover:text-blue-400">→</span>
              {point}
            </li>
          ))}
        </ul>
...
export default function Experience() {
  ...
  return (
    <section id="experience" ref={containerRef} className="py-32 bg-[#121212] relative z-50">
      <BackgroundGlows />
      ...
      >
        <p className="font-playfair italic text-2xl text-white/40 animate-pulse">
          &quot;Currently open to full-time AI Engineering roles & freelance projects.&quot;
        </p>
      </motion.div>
    </section>
  );
