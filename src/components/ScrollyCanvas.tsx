...
export default function ScrollyCanvas({ totalFrames, children }: ScrollyCanvasProps) {
  ...
  const updateLayout = useCallback((img: HTMLImageElement, canvas: HTMLCanvasElement) => {
  ...
  return (
    <div ref={containerRef} className="relative h-[300vh] w-full bg-[#121212]">
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center will-change-transform">
        <canvas
          ref={canvasRef}
          className="absolute inset-0 w-full h-full block scale-[1.01] will-change-transform"
